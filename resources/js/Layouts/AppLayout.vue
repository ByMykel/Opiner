<template>
    <div>
        <div class="min-h-screen flex max-w-6xl mx-auto">
            <nav class="sm:pl-3 flex sm:block fixed p-1 sm:pt-20 bg-gray-200 sm:bg-transparent bottom-0 w-full sm:top-0 sm:w-min justify-around">
                <inertia-link class="w-auto" :href="route('home')">
                    <div class="flex rounded-md p-1 my-1 hover:bg-gray-50" :class="{ 'bg-gray-50' : route().current('home') }">
                        <Icons icon="home" />
                        <span class="px-1 hidden md:block">Home</span>
                    </div>
                </inertia-link>
                <inertia-link :href="route('explore')">
                    <div class="flex rounded-md p-1 my-1 hover:bg-gray-50" :class="{ 'bg-gray-50' : route().current('explore') }">
                        <Icons icon="search" />
                        <span class="px-1 hidden md:block">Explore</span>
                    </div>
                </inertia-link>
                <inertia-link :href="route('notifications')">
                    <div class="flex rounded-md p-1 my-1 hover:bg-gray-50" :class="{ 'bg-gray-50' : route().current('notifications') }">
                ...
                <inertia-link :href="route('user', $page.props.auth)">
                    <div class="flex rounded-md p-1 my-1 hover:bg-gray-50" :class="{ 'bg-gray-50' : route().current('user', $page.props.auth) }">
                        <Icons icon="user" />
                        <span class="px-1 hidden md:block">Profile</span>
                    </div>
                </inertia-link>
                <inertia-link :href="route('profile.show')">
                    <div class="flex rounded-md p-1 my-1 hover:bg-gray-50" :class="{ 'bg-gray-50' : route().current('profile.show') }">
                        <Icons icon="cog" />
